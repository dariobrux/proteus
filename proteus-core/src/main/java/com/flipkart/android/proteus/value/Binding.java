...
//    private static int indexArray = 0;
    ...
    {
        return value.length() > 3 && value.charAt(0) == BINDING_PREFIX_0 && value.charAt(1) == BINDING_PREFIX_1 && value.charAt(value.length() - 1) == BINDING_SUFFIX;
    }
    ...
    public static class DataBinding extends Binding
    {
            ...
            else
            {
                ...
                for (Token token : tokens)
                {
                    ...
                    if (elementToReturn.isArray())
                    {
                        ...
                        {
                            case INDEX:
                                if (index < tempArray.size() && index >= 0)
                                {
                                ...
                                break;
                            case ARRAY_DATA_LENGTH_REFERENCE:
                                elementToReturn = new Primitive(tempArray.size());
                                break;
                            case ARRAY_DATA_LAST_INDEX_REFERENCE:
                                if (tempArray.size() == 0)
                                {
                                    return Result.NO_SUCH_DATA_PATH_EXCEPTION;
                                }
        ...
        public String toString()
        {
            ...
            return new StringBuilder().append(BINDING_PREFIX_0)
